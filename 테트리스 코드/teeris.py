...
def draw_window(screen, grid, current_piece, next_piece, score):
    ...
    # 5. [신규 기능] 다음 블록 미리보기(NEXT) 표시
    next_label = font.render("NEXT", True, WHITE)
    screen.blit(next_label, (GRID_WIDTH * BLOCK_SIZE + 20, 200))
    
    PREVIEW_BLOCK_SIZE = 20 # 미리보기용 대기 블록은 20px 크기로 아담하게 조정
    if next_piece:
    ...
    while not game_over:
        ...
        if fall_time >= fall_speed:
            ...
            if not check_collision(current_piece, grid, adj_y=1):
                ...
                # [기능 변경] 다음 대기 블록을 현재 블록으로 변경하고, 새로운 대기 블록 생성
